...
@section('content')
<div class="row">
    <form action="{{ url('email/send') }}" class="validate" autocomplete="off" method="post" accept-charset="utf-8">
		<div class="col-md-8">				
		...
		$.ajax({
			...
			beforeSend: function(){
				...
				jQuery.each( json, function( i, val ) {
					$('select[name=user_id]').append("<option value='"+val['email']+"'>"+val['name']+"</option>");
				});
			...
			$.ajax({
				...
				beforeSend: function(){
					...
					jQuery.each( json, function( i, val ) {
					   ...
					   .append('<div class="col-md-12">'+
									'<label class="c-container">'+
									   '<input type="checkbox" value="'+val['email']+'" name="users[]" checked="true">'+val['name']+
									   '<span class="checkmark"></span>'+
									'</label>'+
								'</div>');
					});
